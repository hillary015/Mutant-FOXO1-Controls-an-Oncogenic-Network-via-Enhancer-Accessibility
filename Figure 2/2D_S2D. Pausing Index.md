...
```{r}
mako(4)
bppalette <- c("#0B0405FF", "#4ACDCF", "#DEF5E5FF")

stackedbp <- function(data, title, pal) {
  ggplot(data, aes(x = cluster, y = percent, fill = group)) +
  geom_bar(width = 0.75, position = "stack", stat ="identity", color = "black", size = 0.5) +
  ggtitle(title) +
  xlab("Cluster") +
  ylab("Percent") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100.01)) +
  theme_hillary() +
  theme(panel.border = ggplot2::element_rect(colour = "black", fill = NA, size = 0.75)) +
  coord_flip(clip = "off") +
  scale_x_reverse() +
...
bp_0.5 <- stackedbp(siggenes_wpichange_cluster_count_0.5, "LY1 Sig PI 0.5h", bppalette)
bp_0.5
bp_1 <- stackedbp(siggenes_wpichange_cluster_count_1, "LY1 Sig PI 1h", bppalette)
bp_1
bp_2 <- stackedbp(siggenes_wpichange_cluster_count_2, "LY1 Sig PI 2h", bppalette)
bp_2
bp_N1 <- stackedbp(siggenes_wpichange_cluster_count_N1, "NUD Sig PI 1h", bppalette)
bp_N1
bp_N2 <- stackedbp(siggenes_wpichange_cluster_count_N2, "NUD Sig PI 2h", bppalette)
bp_N2
bp_D0.5 <- stackedbp(siggenes_wpichange_cluster_count_D0.5, "DHL4 Sig PI 0.5h", bppalette)
bp_D0.5
bp_D1 <- stackedbp(siggenes_wpichange_cluster_count_D1, "DHL4 Sig PI 1h", bppalette)
bp_D1
bp_D2 <- stackedbp(siggenes_wpichange_cluster_count_D2, "DHL4 Sig PI 2h", bppalette)
bp_D2

bp_noclus <-  ggplot(siggenes_wpichange_nocluster, aes(x = cell_line, y = percent, fill = group)) +
  geom_bar(width = 0.75, position = "stack", stat ="identity", color = "black", size = 0.5) +
  ggtitle("PI Change") +
  xlab("Cell Line") +
  ylab("Percent") +
...
## Print to png
```{r}
cairopng_4x2 <- function(title, object) {
...
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_LY10.5_percentpercluster_bargraph_4x2.png",bp_0.5)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_LY11_percentpercluster_bargraph_4x2.png",bp_1)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_LY12_percentpercluster_bargraph_4x2.png",bp_2)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_NUD1_percentpercluster_bargraph_4x2.png",bp_N1)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_NUD2_percentpercluster_bargraph_4x2.png",bp_N2)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_DHL40.5_percentpercluster_bargraph_4x2.png",bp_D0.5)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_DHL41_percentpercluster_bargraph_4x2.png",bp_D1)
dev.off()
cairopng_4x2("20230906 NUD_LY1_PS_PIclass_DHL42_percentpercluster_bargraph_4x2.png",bp_D2)
dev.off()

Cairo::Cairo(file = "20230906 NUD_LY1_PS_PIclass_nocluster_bargraph_6x3.png", 
             bg = "white",
             type = "png",
             units = "in", 
             width = 6, 
